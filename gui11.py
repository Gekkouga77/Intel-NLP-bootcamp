...
# Menu bar
    ...
    # File
        ...
        help_option.add_command(label="About AstroBot", command=self.msg)
    ...
    def msg(self):
        tkinter.messagebox.showinfo("AstroBot v7.7answer queries",'AstroBot is a chatbot for answering queries on the solar system\nIt is based on retrival-based NLP using pythons NLTK tool-kit module\nGUI is based on Tkinter\nIt can answer questions regarding the solar system for new learners')

    def about(self):
        tkinter.messagebox.showinfo("AstroBot Developers","Vignesh N")

    
    def send_message_insert(self, message):
        user_response = self.entry_field.get()
        pr1 = "Human : " + user_response + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, pr1)
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)
        ob=chat(user_response)
        pr="Master678: " + ob + "\n"
        self.text_box.configure(state=NORMAL)
...
root.title("AstroBot: The Solar System Assistant")
root.mainloop()
